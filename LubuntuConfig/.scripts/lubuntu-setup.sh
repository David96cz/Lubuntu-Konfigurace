...
sudo apt install --no-install-recommends xfwm4 xfconf gdebi flameshot htop vlc featherpad icoutils viewnior ffmpegthumbnailer heif-gdk-pixbuf zram-tools gthumb cabextract libgl1-mesa-dri:i386 libgl1:i386 libglx-mesa0:i386 yad cups-client cups brightnessctl libnotify-bin copyq -y
...
# Pokud je soubor zkratek prázdný (čistá instalace), překopírujeme tam výchozí systémový,
# abychom měli v čem vyhledávat a nahrazovat.
if [ ! -s "$SHORTCUTS_CONF" ]; then
    cp /usr/share/lxqt/globalkeyshortcuts.conf "$SHORTCUTS_CONF" 2>/dev/null
fi

# Brutální přepsání výchozí utility na náš skript (argumenty -i a -d zůstanou zachovány)
sed -i 's/lxqt-config-brightness/brightness.sh/g' "$SHORTCUTS_CONF"

# Přidání uživatele do skupiny video, aby brightnessctl běžel bez hesla
sudo usermod -aG video $USER
echo "   [OK] Ovládání jasu přemostěno."

# KRITICKÝ KROK: Přidání uživatele do skupiny video, aby brightnessctl fungoval bez hesla
sudo usermod -aG video $USER
...
# ---------------------------------------------------------
# 12. KOPÍROVÁNÍ VLASTNÍCH SKRIPTŮ DO SYSTÉMOVÉ CESTY (~/.local/bin)
# ---------------------------------------------------------
echo ">> Nasazuji custom skripty z .scripts do ~/.local/bin..."

# Cesta k aktuální složce, odkud spouštíš tento setup skript (tedy složka .scripts na flashce)
SCRIPTS_SOURCE="$(dirname "$(realpath "$0")")"

if [ -d "$SCRIPTS_SOURCE" ]; then
    # Vytvoření cílové složky pro uživatelské binárky
    mkdir -p "$LOCAL_BIN"
    
    # Zkopíruje úplně všechny .sh a .py skripty (kromě samotného setupu) do .local/bin
    # -u zajistí, že to přepisuje jen pokud je zdroj novější
    cp -u "$SCRIPTS_SOURCE/"*.sh "$LOCAL_BIN/" 2>/dev/null
    cp -u "$SCRIPTS_SOURCE/"*.py "$LOCAL_BIN/" 2>/dev/null
    
    # Abychom si do systému nedali i samotný instalační skript, rovnou ho z binárního adresáře smažeme
    rm -f "$LOCAL_BIN/$(basename "$0")"
    
    # Nastavení spustitelnosti pro vše ve složce bin
    chmod +x "$LOCAL_BIN/"* 2>/dev/null
    
    # --- AUTOSTART PRO UPDATE-WRAPPERS.SH ---
    echo ">> Přidávám update-wrappers do autostartu..."
...
EOF
    update-desktop-database "$USER_APPS_DIR" 2>/dev/null
    echo "   [OK] Skripty nasazeny do PATH a zástupce vytvořen."
    ...
    echo "   [!] CHYBA: Zdrojová složka se skripty nebyla nalezena!"
fi
...
# Poznámka: Tady jsi nechal původní vložení do cronu. Pokud tvůj "autoupdate.sh" dělá to samé, 
# mohl bys tenhle blok smazat a nechat v /etc/cron.daily/ jen symlink na ten tvůj nový skript.
...
# 18. AUTOSTART COPYQ
# ---------------------------------------------------------
echo ">> Nastavuji autostart pro CopyQ..."
...
echo "   [OK] CopyQ přidán do autostartu."
